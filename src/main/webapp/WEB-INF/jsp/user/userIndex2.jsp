    ...
    <head>
        ...
        <script>
            $(function () {
                ...
                $("#findType").blur(function (){
                    let findType =$("#findType option:selected").val();
                    if (findType=="bookName"){
                        $("#form-id").attr("action","${pageContext.request.contextPath}/book/findBookByBookName")
                    }
                    if (findType=="author"){
                        $("#form-id").attr("action","${pageContext.request.contextPath}/book/findBookByAuthor")
                    }
                    if (findType=="publisher"){
                        $("#form-id").attr("action","${pageContext.request.contextPath}/book/findBookByPublisher")
                    }
                })
            })
    ...
    <div class="main-context">
        ...
        <div class="row" style="margin-top: 20px;">
            <div class="col-lg-9 col-lg-offset-2">
                <form class="form-inline" method="get" action="${pageContext.request.contextPath}/book/findBook" id="form-id">
                    <select class="form-control op" style="width: 150px;" name="type" id="findType">
        ...
        <div class="show-table" style="box-shadow: 0 4px 12px rgb(0 0 0);">
            <table class="table table-striped table-hover" style="border: 1px solid #ccc;">
                ...
                <tbody>
                <c:forEach items="${pageInfo.extend.pageInfo.list}" var="book">
                    <tr>
...
<%--            分页栏--%>
            <div class="page-nav"><nav aria-label="Page navigation">
                <ul class="pagination">
                    <c:if test="${pageInfo.extend.pageInfo.pageNum==1}">
                        <li class="disabled">
                            <span >
                    ...
                    <c:if test="${pageInfo.extend.pageInfo.pageNum!=1}">
                        <li>
                            <a href="${pageInfo.requestAddress}?pn=${pageInfo.extend.pageInfo.pageNum-1}&type=${pageInfo.type}&data=${pageInfo.data}" aria-label="Previous" >
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>
                    </c:if>

                    <c:forEach items="${pageInfo.extend.pageInfo.navigatepageNums}" var="i">
                        <c:if test="${pageInfo.extend.pageInfo.pageNum==i}">
                            <li class="active"><a href="${pageInfo.requestAddress}?pn=${i}&type=${pageInfo.type}&data=${pageInfo.data}">${i}</a></li>
                        </c:if>
                        <c:if test="${pageInfo.extend.pageInfo.pageNum!=i}">
                            <li><a href="${pageInfo.requestAddress}?pn=${i}&type=${pageInfo.type}&data=${pageInfo.data}">${i}</a></li>
                        </c:if>
                    </c:forEach>

                    <c:if test="${pageInfo.extend.pageInfo.pageNum==pageInfo.extend.pageInfo.pages}">
                        <li class="disabled">
                            <span>
                    ...
                    <c:if test="${pageInfo.extend.pageInfo.pageNum!=pageInfo.extend.pageInfo.pages}">
                        <li>
                            <a href="${pageInfo.requestAddress}?pn=${pageInfo.extend.pageInfo.pageNum+1}&type=${pageInfo.type}&data=${pageInfo.data}" aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
                    </c:if>
